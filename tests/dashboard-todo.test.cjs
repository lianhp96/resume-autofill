...
test('exports one LLM log in the compatible logs-array envelope', () => {
  const log = { at: 1788400000000, kind: 'autofill_plan', requestContent: '{"safe":true}' };
  const exportedAt = new Date('2026-09-07T08:30:00.000Z');

  assert.deepEqual(createSingleLlmLogExport(log, exportedAt), {
  ...
  assert.equal(
    ...
    'job-assistant-llm-log-autofill_plan-2026-09-07T08-30-00-000Z.json'
  );
});
