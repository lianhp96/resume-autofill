...
准确率优先于覆盖率，禁止为了覆盖页面字段而猜测或强行匹配。只有两个字段表达同一项数据时才能映射；同属一个类别、都是地点/身份/联系方式，或名称相近，都不足以构成映射。没有可靠对应资料时必须跳过该页面字段。
例如，民族不能映射到性别，籍贯不能映射到户籍地址，学历不能映射到学位，家庭成员工作单位不能映射到实习单位，家庭成员联系电话不能映射到本人手机，期望城市不能映射到求职意向。输入中的 context 是网页表单的父级标签路径，section 和 repeatIndex 是资料字段的结构信息；它们只能用于消歧，不能用来推断不存在的对应资料。
